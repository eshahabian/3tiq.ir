...
const mountainIcon = L.divIcon({
    ...
    "><span style="transform:rotate(45deg); font-size:0.875rem;">⛰️</span></div>`,
    className: '',
    iconSize: [32, 32],
    iconAnchor: [16, 32],
    popupAnchor: [0, -36]
...
mountainPanel.innerHTML = `
    <h4 style="margin:0 0 8px; color:#2d6a4f; font-size:0.81rem;">⛰️ جستجوی کوه</h4>
    <input id="mountain-search" type="text" placeholder="نام کوه..." style="
        width:100%; padding:6px 8px; border:1px solid #ddd;
        border-radius:8px; font-size:0.81rem; box-sizing:border-box;
    ...
    <select id="mountain-filter" style="
        ...
        font-size:0.81rem; font-family:inherit; direction:rtl; outline:none;
    ">
    ...
    <div id="mountain-count" style="margin-top:8px; font-size:0.69rem; color:#888; text-align:center;"></div>
`;
        ...
        function renderMountains(list) {
            ...
            list.forEach(m => {
                const marker = L.marker([m.lat, m.lng], { icon: mountainIcon })
                    .addTo(map)
                    .bindPopup(`
                        <div style="text-align:center; font-family:'Vazirmatn',Tahoma; direction:rtl; min-width:9rem; padding:4px;">
                            <b style="font-size:0.875rem; color:#2d6a4f;">⛰️ ${m.name}</b><br>
                            <span style="color:#555; font-size:0.75rem;">ارتفاع: <b>${m.height.toLocaleString('fa-IR')}</b> متر</span><br>
                            <span style="color:#888; font-size:0.69rem;">📍 ${m.province}</span>
                        </div>
                    `);
...
const shelterIcon = L.divIcon({
    html: `<div style="
        ...
        box-shadow: 0 2px 8px rgba(0,0,0,0.4);
        border: 2px solid white;
    "><span style="transform:rotate(45deg); font-size:0.75rem;">🏕️</span></div>`,
    className: '',
    iconSize: [28, 28],
    iconAnchor: [14, 28],
    popupAnchor: [0, -32]
...
shelterPanel.innerHTML = `
    <h4 style="margin:0 0 8px; color:#d97706; font-size:0.81rem;">🏕️ جستجوی پناهگاه</h4>
    ...
    <div id="shelter-count" style="margin-top:8px; font-size:0.69rem; color:#888; text-align:center;"></div>
`;
        ...
        function renderShelters(list) {
            ...
            list.forEach(s => {
                const marker = L.marker([s.lat, s.lng], { icon: shelterIcon })
                    ...
                    .bindPopup(`
                        <div style="text-align:center; font-family:'Vazirmatn',Tahoma; direction:rtl; min-width:9rem; padding:4px;">
                            <b style="font-size:0.875rem; color:#d97706;">🏕️ ${s.name}</b><br>
                            <span style="color:#555; font-size:0.75rem;">نوع: <b>${s.type}</b></span><br>
                            <span style="color:#888; font-size:0.69rem;">📍 ${s.province}</span>
                        </div>
                    `);
